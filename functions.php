...
function signUp($full_name,$person_adress,$person_cnic,$account_type,$amount,$pincode)
{
    $sql="SELECT * FROM person WHERE cnic="."$person_cnic";
        ...
        if($row_count==0)
        {
           ...
           $id=$person_id["id"];
           $sql="INSERT INTO accounts(account_type,pincode,person_id) VALUES($account_type,$amount,$pincode,$id) ";
           $res=executeQuery($link, $sql);
           $sql="SELECT account_number FROM accounts WHERE person_id=$id";
           $account_num_res=executeQuery($link, $sql);
           $account_num=mysqli_fetch_assoc($account_num_res);
           $account_number=$account_num["account_number"];
           $sql="INSERT INTO transactions(deposits,balance,account_number) VALUES($amount,$amount,$account_number) ";
           mysqli_commit($link);
           if($res)
...
function deposit($description,$balance,$transaction,$account_number)
{
